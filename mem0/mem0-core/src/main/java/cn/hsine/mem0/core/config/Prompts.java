...
/**
 * All prompt templates for mem0, ported from Python mem0/configs/prompts.py.
 *
 * @author MoBai
...
public final class Prompts {
    ...
    public static final String FACT_RETRIEVAL_PROMPT = """
        ...
        - If you ask where you fetched my information, answer that you found from publicly available sources on internet.
    ...
    public static final String USER_MEMORY_EXTRACTION_PROMPT = """
    ...
    public static final String AGENT_MEMORY_EXTRACTION_PROMPT = """
    ...
    public static final String PROCEDURAL_MEMORY_SYSTEM_PROMPT = """
        ...
        3. **Detail and Precision**:
           ...
           - For any errors, include the full error messageEntity and, if applicable, the stack trace or cause.
    ...
    // ── ADDITIVE_EXTRACTION_PROMPT (V3) ──────────────────────────────────

    public static final String ADDITIVE_EXTRACTION_PROMPT = """
        ...
        **When in doubt, extract.** A slightly redundant memory is far less costly than a missing one. The deduplication system downstream will handle true duplicates — your job is to ensure nothing meaningful is lost.
        ...
        When a user asks a question or makes a request, their messageEntity often contains INCIDENTAL PERSONAL FACTS stated as context. These facts are just as extractable as the request itself:
        ...
        **IMPORTANT — Extract ALL dimensions of a conversation.** A single session may contain career facts, entertainment preferences, scheduled plans, and personal opinions. Extract each dimension as a separate memory. Do not let one dominant topic cause you to miss secondary information.
        ...
        When a messageEntity contains a photo description (e.g., "[Shared photo: ...]" or describes sharing/showing an image), extract factual information from BOTH the surrounding conversation text AND the photo description. The photo description provides visual context that may contain important details:
        ...
        - **No Echo Extraction**: When an assistant messageEntity restates, summarizes, or confirms information the user already provided in the same conversation, do NOT extract it again from the assistant's messageEntity. Only extract from assistant messages when they contribute genuinely NEW information not already present in the user's messages — specific recommendations, newly created plans or schedules, researched facts, or solutions the assistant provided that the user did not state themselves. If the user says "I want daily check-ins at 7:30 AM" and the assistant responds "I've set up daily check-ins at 7:30 AM", that is already captured from the user's messageEntity — do not extract a second memory from the assistant's echo.
        ...
        - **No Detail Contamination from Context**: When extracting from New Messages, do NOT import or merge details from Existing Memories or Recent Memories into the new extraction UNLESS the new messageEntity explicitly references those details. If the New Message says "I had a great meal" and an Existing Memory says "User's favorite restaurant is Olive Garden," do NOT produce "User had a great meal at Olive Garden" — the new messageEntity never mentioned the restaurant. Each extraction must be faithful to its source messageEntity only.
        ...
        Before producing output, mentally scan the ENTIRE conversation — every single messageEntity — and verify:
        ...
        4. Re-read each user messageEntity individually: does EVERY specific fact, preference, experience, or event mentioned in that messageEntity have a corresponding extraction? If a single messageEntity mentions two distinct facts (e.g., an allergy AND a hobby), both must be captured.
    ...
    public static final String AGENT_CONTEXT_SUFFIX = """
    ...
    public static String generateAdditiveExtractionPrompt(
            Object summary,
            List<?> recentlyExtractedMemories,
        ...
        String[] dates = resolveDates(currentDate, timestamp);
        String resolvedCurrentDate = dates[0];
        String observationDate = dates[1];

        StringBuilder sb = new StringBuilder();

        sb.append("## Summary\n").append(formatSummary(summary));
        appendSection(sb, "## Last k Messages", formatConversationHistory(lastKMessages));
        appendSection(sb, "## Recently Extracted Memories", serializeMemories(recentlyExtractedMemories));
        appendSection(sb, "## Existing Memories", serializeMemories(existingMemories));
        appendSection(sb, "## New Messages", formatNewMessages(newMessages));
        appendSection(sb, "## Observation Date", observationDate);
        appendSection(sb, "## Current Date", resolvedCurrentDate);

        if (customInstructions != null && !customInstructions.isEmpty()) {
            appendSection(sb, "## Custom Instructions", customInstructions);
        }

        if (useInputLanguage) {
            appendSection(sb, "## Language Requirement",
    ...
    public static final int PAST_MESSAGE_TRUNCATION_LIMIT = 300;

    // ── Helper: format today's date for prompts with %s placeholder ──────

    private static final DateTimeFormatter DATE_FMT = DateTimeFormatter.ISO_LOCAL_DATE;

    /**
     * Returns AGENT_MEMORY_EXTRACTION_PROMPT with today's date interpolated.
     */
    public static String agentMemoryExtractionPrompt() {
        return String.format(AGENT_MEMORY_EXTRACTION_PROMPT, LocalDate.now().format(DATE_FMT));
    }
    ...
    private static String truncateContent(String text, int limit) {
        if (text == null) {
            return "";
        }
    ...
    private static String formatSummary(Object summary) {
        if (summary instanceof Map) {
            Object s = ((Map<?, ?>) summary).get("summary");
            return s != null ? s.toString() : "";
        }
        return summary != null ? summary.toString() : "";
    }

    private static String formatConversationHistory(List<?> messages) {
        ...
        for (Object msg : messages) {
            if (msg instanceof Map) {
                ...
                Object content = m.get("message");
                if (content == null) {
                    content = m.get("content");
                }
                if (role != null && content != null) {
                    result.append(role).append(": ")
                            .append(truncateContent(content.toString(), PAST_MESSAGE_TRUNCATION_LIMIT))
                            .append("\n");
                }
            }
        }
    ...
    private static String serializeMemories(List<?> memories) {
    ...
    private static String formatNewMessages(Object newMessages) {
        if (newMessages instanceof String) {
            return (String) newMessages;
        }
        try {
            com.fasterxml.jackson.databind.ObjectMapper mapper = new com.fasterxml.jackson.databind.ObjectMapper();
            return mapper.writeValueAsString(newMessages != null ? newMessages : "[]");
        } catch (Exception e) {
            return "[]";
        }
    }

    private static String[] resolveDates(String currentDate, String observationDate) {
        ...
        return new String[]{currentDate, observationDate};
    }
}
